...
- **Supabase + RLS (pegadinha):** `PATCH`/`UPDATE` **e `DELETE`** bloqueados pelo RLS devolvem **HTTP 204 sem erro** com 0 linhas afetadas — parecem sucesso e não são. Sempre **reler o registro** depois de gravar/apagar (`?id=eq.X&select=...`).
- **Mapa de permissões da anon key** (testado em 18/09): `arquivo` → SELECT/**INSERT** (é assim que a publicação funciona); `contatos` → INSERT sim, SELECT **não** (privacidade — o registro existe mas não aparece em consulta); `depoimentos` e `parceiros` → **INSERT bloqueado pelo RLS** (401, `42501`). ✅ **RESOLVIDO (18/09):** a `service_role` fica em `C:/Users/PRE-IMPRESSOR/R2/.supabase-sagi.env` (fora do repositório, junto do `.r2-sagi.env`) e os scripts `cadastrar.py` / `excluir.py` a leem por `scripts/supabase_admin.py` — confira com `python scripts/cadastrar.py --testar-chave`
- **Teste de mídia/INSERT sempre suja o banco:** publicar ou inserir para testar cria registro que só sai com SQL. Antes de publicar teste, validar a lógica **fora do site** (script Python que repete a consulta + o casamento de chaves, como `scripts/_teste_parteB.py`, ou Node rodando a própria função do `main.js`, como `scripts/_teste_card_depoimento.js`) — e só publicar quando for foto real do cliente
