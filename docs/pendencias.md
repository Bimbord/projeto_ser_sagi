...
- **Supabase + RLS (pegadinha):** um `PATCH`/`UPDATE` bloqueado pelo RLS devolve **HTTP 204 sem erro** com 0 linhas afetadas — parece sucesso e não é. Sempre **reler o registro** depois de gravar (`?id=eq.X&select=...`). Na prática: a anon key faz SELECT/INSERT, mas **não** atualiza `arquivo`; para editar ou marcar `deleted=true` é preciso SQL no painel (service_role)
